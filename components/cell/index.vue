<template>
	<view :class="['i-class','i-cell',isLastCell ? 'i-cell-last' : '' ,isLink ? 'i-cell-access' : '' ]"
		@tap="navigateTo"
	>
		...
		<view @tap.stop="navigateTo" class="i-cell-ft">
...
<script>
	export default {
		...
		methods:{
            ...
            const type = typeof this.$props.isLink;
            if (!this.$props.isLink || !url || url === 'true' || url === 'false') return;

            if (type !== 'boolean' && type !== 'string') {
                warn('isLink 属性值必须是一个字符串或布尔值', this.$props.isLink);
                return;
            }
			...
			console.log(type,"islink")
            uni[this.$props.linkType]({
				url:url
			});
        },
        handleTap () {
